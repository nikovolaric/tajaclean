...
import {
  getTopProductsByLastDays,
  getTopProductsByMonth,
} from "@/lib/orderActions";
import { ChevronDown } from "lucide-react";
import { useEffect, useState } from "react";
import { useStatsContext } from "../stats/StatsContextProvider";
...
export function MostSoldItemsDays() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-xl font-semibold">Najbolj prodajani izdelki</p>
      <div className="flex h-full flex-col gap-4 rounded-xl bg-white p-5 shadow-[0px_1px_2px_rgba(0,0,0,0.25)]">
        {isLoading ? (
          <Spinner />
        ) : (
          items.map((el: { product_name: string; total_price: number }) => (
