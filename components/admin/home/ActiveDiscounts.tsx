...
async function ActiveDiscounts() {
  const data = { results: 0, discounts: [] };
      ...
      <div className="flex h-full flex-col justify-between gap-4 rounded-xl bg-white p-5 shadow-[0px_1px_2px_rgba(0,0,0,0.25)]">
        ...
        {data.results > 0 && (
          <ul className="flex flex-col gap-8">
            {data.discounts.map(
              (discount: { _id: string; name: string; used: number }) => (
                <Suspense key={discount._id} fallback={<p>Loading...</p>}>
        ...
        <LinkBtn variant="primary" href="/admin/popusti">
